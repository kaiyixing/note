...
调优前必须通过`top/htop`、`ss/netstat`（查看网络状态）、`vmstat`,`iostat`(查看整体状态)等工具确定真实瓶颈（CPU/内存/磁盘I/O/网络I/O），避免盲目修改。详见 [[Linux系统资源紧张排查学习指南]] 中的排查思路。
...
- **高并发Web**：优化TCP连接队列、端口复用（参考 [[Kubernetes知识梳理]] 中容器网络性能调优）
...
- [[Linux系统资源紧张排查学习指南]] — 内核调优前置排查：定位 CPU/内存/磁盘/网络瓶颈
- [[Linux 故障排查实操指南（详细步骤）]] — 故障排查与内核调优配合使用
- [[Kubernetes知识梳理]] — 容器网络与内核参数调优的关联
- [[Linux 系统核心组件配置文件速查表]] — 系统配置文件路径与调优相关
- [[面试问答]] — 面试中内核调优相关高频问题
- [[面试准备/面经/大厂运维运维开发面试高频知识点总结]] — 运维面试高频知识点整合