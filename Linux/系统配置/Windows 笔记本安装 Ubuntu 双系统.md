...
### 2.5 重装整个 Ubuntu 系统（替换旧系统）

已经有一个 Ubuntu，现在想装个新的（换版本、系统坏了想重来），流程是：**先在 Windows 里删掉旧 Ubuntu 分区，再正常装新的**。

**第一步：在 Windows 里删掉旧的 Ubuntu 分区**

进 Windows → `Win + X` → 磁盘管理。

找到 Ubuntu 那几个**没有盘符**的分区（显示为 RAW 或 Ext4）。依次右键 → **删除卷**：
...
**不要碰的：**
- C 盘（Windows 系统盘，NTFS）
- EFI 系统分区（FAT32，几百 MB）—— 留着，新系统继续用
- Windows 恢复分区

删完后这些区域变成 **未分配** 空间，装新 Ubuntu 时直接用。

> 如果磁盘管理里看不到 Ubuntu 分区，用 DiskPart：
> ```
> Win + R → diskpart
> list disk
> select disk X
> list partition
> select partition Y    ← Ubuntu 分区号
> delete partition override
> ```

**第二步：清理 EFI 里的旧 Ubuntu 引导（推荐）**
...
插 U 盘重启，按正常流程装（同 2.1→2.4）：

- 手动分区时，之前删出来的**未分配空间**就是新系统的地盘
- `/`、`/home`、swap 在新空间里重新分
- EFI 分区选已有的那个，挂载 `/boot/efi`，**不要格式化**
- 装完 GRUB 自动识别 Windows，双引导正常

> 旧系统和 `/home` 的数据在删分区时已经没了。有要保留的文件，操作前先拷到 Windows 盘备份。
