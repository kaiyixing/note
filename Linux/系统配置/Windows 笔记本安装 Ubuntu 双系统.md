...
重装 Ubuntu（不删 Windows）时，跟第一次有些区别，注意这几点：

**① 先备份**
重装会格式化 `/` 分区。`/home`、`/opt`、`/usr/local` 里的数据如果有重要的，先拷到 Windows 盘或 U 盘里。

**② 分区要认准，别删错盘**
手动分区时，**仔细看分区大小和类型**，只动 Ubuntu 的分区，Windows 的分区（NTFS）和 EFI 分区碰都不要碰。

**③ EFI 分区不要格式化**
已有 Ubuntu 引导的情况下，EFI 分区里已经存了引导文件。安装时：

- 选中已有的 EFI 分区，挂载点设为 `/boot/efi`
- **不要勾选"格式化"**
- 否则 Windows 引导也会被干掉
...
|------|---------|------|
| `/` | ✅ **格式化** | 装新的系统，旧文件必须清掉 |
| `/home` | ❌ **不格式化**（只挂载） | 保留之前的个人文件、配置、下载内容 |

如果 `/home` 是单独分区：挂载时选它，**不勾格式化**，挂到 `/home`。
如果 `/home` 之前没单独分：那就只能格式化根分区，数据全部丢失。

**⑤ 引导器位置选对**
同样选那个 ESP 分区（FAT32，挂载为 `/boot/efi`），安装程序会覆盖旧的 Ubuntu 引导文件，但不影响 Windows。

**⑥ 用户名可以和上次一样**
安装时设置同一个用户名和密码，权限配置会保持一致。如果 `/home` 是保留的，还能直接复用旧的家目录。
